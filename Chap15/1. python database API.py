...
import sqlite3      # Database fully contained in 1 file

def main():
    print('connect')
    db = sqlite3.connect('db-api.db')   # Conenct to database API to return database handle
    cur = db.cursor()       # Get cursor
    print('create')
    cur.execute("DROP TABLE IF EXISTS test")    # Execute SQL code in speech marks
    cur.execute("""
        CREATE TABLE test (
